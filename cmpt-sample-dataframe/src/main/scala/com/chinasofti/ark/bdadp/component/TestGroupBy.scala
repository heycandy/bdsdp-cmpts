...
object TestGroupBy {
  ...
  def main(args: Array[String]) {
    ...
    val input = options.getParameter.getOrDefault("pipeline.input",
      """[{"name": "xiao", "age": 25,"work": 2,"sex":"male"},{"name": "xiao", "age": 25,"work": 2,"sex":"female"},{"name": "xiao", "age": 50,"work": 2,"sex":"male"}]""")
    val transform = options.getParameter.getOrDefault("pipeline.transform",
      """[{"id": "1", "name": "groupBy", "colNames": "name,sex","aggExpr":"max(age),sum(work)"}]""")
    ...
    val pipeline = mapper.readValue[Seq[TransformModel]](transform).map(f => {
      ...
      props.setProperty("colNames", f.colNames)
      props.setProperty("aggExpr", f.aggExpr)
  ...
  case class TransformModel(id: String, name: String, colNames: String, aggExpr: String)
